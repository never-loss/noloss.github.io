// Pesquisa em velas reais só com operações executáveis nos multiplicadores da Deriv. Sem login, sem compras.
// Uso: node src/cli/research-executable.ts --granularity 3600 --candles 6000 --min-multiplier 100
//      node src/cli/research-executable.ts --all-crypto
...
if (!(minMultiplier > 0)) {
  console.error("--min-multiplier tem de ser positivo");
  process.exit(1);
}
...
async function main(): Promise<void> {
  await client.ready;
  console.log(
    `Ligado. Velas de ${granularity}s, até ${target} por símbolo. Só operações executáveis (multiplicador mínimo x${minMultiplier}). Concorrência ${concurrency}.`,
  );
  ...
  const result = runExecutableResearch(data, {
    slAtr,
    tpR,
    maxBars,
    trainSize,
  ...
  console.log("");
  console.log(formatShares(result));
  console.log(formatMarketReport(result.groups));
}
